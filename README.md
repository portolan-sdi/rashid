...
| `PTL-ROL` | 001–002 | a raster collection with scene items publishes an `items.parquet` rollup (a warning, per the SHOULD), and a published rollup carries both registrations: the `geoparquet-items` asset with role `stac-items` and a `rel:"items"` link, both typed `application/vnd.apache.parquet` |
