...
`PTL-COL-006` reports a collection that refers to no data. The reference takes one of three forms: a collection asset with the `data` role, item assets with the `data` role, or `partition:glob`. The rule fires only on a collection with no items, so it never overlaps `PTL-AST-007`, which reports each item without a `data` asset. A collection whose scene files or item mirror imply items is `PTL-COL-005`'s finding, because there the repair is the items and not the asset list. If collection assets omit `roles`, rashid cannot decide whether the rule applies, and `PTL-AST-001` reports that omission instead.
...
| `PTL-AST` | 001–007 | Checks asset `href`, media type, and roles. Requires the COG media type for primary raster data and HTTPS instead of S3 URLs. Requires `file:size` and multihash-encoded `file:checksum`; their absence produces a warning under the SHOULD requirement. Prohibits assets on catalogs. Requires every item to carry at least one asset with the `data` role. |
...
| `PTL-COL` | 001–006 | Requires a single-file collection to expose data through a collection asset instead of one item. Prohibits nested collections. Requires collection IDs to follow the naming convention and remain unique within their language tree. Requires raster scenes to appear on items instead of the collection. Requires a collection to publish the items that its scene files or its item mirror imply. Requires a collection to refer to its primary data. |
