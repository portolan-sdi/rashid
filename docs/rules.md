...
- `PTL-EXT-002` produces an info finding when an object declares an extension the profile's registry does not list. The finding records that rashid did not validate against that schema. It does not claim the object is wrong.
- `PTL-EXT-000` produces a warning when one extension schema cannot be fetched or resolved. rashid reports it once for each URI and validates every other extension.
...
| `PTL-EXT` | 000–002 | Checks every object against the schemas of the STAC extensions it declares. Pinned versions use packaged schemas. A registered extension at another version uses its fetched schema. Rule `000` warns when the pass or one schema cannot load. Rule `002` reports once per catalog that an unregistered extension was not validated. `--schema-allow-network` fetches and validates it instead. |
...
| `PTL-LIV` | 000–006 | Checks the hosting server's Data Storage MUST requirements for absolute HTTPS asset URLs. For each host, checks ranged GET support through `206` and `Accept-Ranges: bytes`, `Content-Length` on HEAD, CORS origins, exposed headers, and preflight support. Preflight must allow `GET`, `HEAD`, `Range`, and conditional-request headers. HEAD size must match `file:size`. Source and alternate assets are exempt. With `--live-base-url`, rule `006` sends one HEAD for each distinct link target under the publish base and reports each target that does not answer `2xx`. This covers `child`, `item`, `parent`, `root`, `self`, `agents`, `describedby`, and every other link that resolves under the base. Links to other hosts are not probed. Rule `000` warns when no asset is available to probe or a host is unavailable. |
...
| Live | Two requests for each host, plus one HEAD for each distinct asset URL. With `--live-base-url`, one HEAD for each distinct link target under the base. | Response headers and one byte for the range check. |
